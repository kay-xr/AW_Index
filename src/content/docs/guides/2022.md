...
Ensure `Reload Scene` is **DISABLED**!
I have no idea when this trend started with creators in the VRChat community, but enabling `Reload Scene` **WILL** cause issues with various tools. To name a few:
  - VRCFury (Will never launch / run in play mode)
  - Modular Avatar (The same as the former, though I haven't tested it personally.)

> `Reload Domain` works fine with *most* tools. You can enable this here to speed up playmode launch pretty dramatically, as most well-made tools should handle things accordingly.
